...
<div class="max-w-sm bg-white rounded-2xl shadow-md overflow-hidden border {{ $enrolled ? 'border-green-500' : 'border-gray-200' }}">
    <a href="{{ route('skill-development.show', $course->slug) }}">
        <div class="relative">
            ...
            @if($enrolled)
            <div class="absolute top-3 right-3 bg-green-100 text-green-700 font-semibold text-sm px-3 py-1 rounded-full shadow flex items-center gap-1">
            ...
            @else
            <div class="absolute top-3 right-3 bg-white text-blue-600 font-semibold text-sm px-3 py-1 rounded-full shadow">
